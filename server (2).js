...
app.post('/api/scoreOpenQuestion', async (req, res) => {
  try {
    ...
    // Use prompt from frontend if provided (contains two-step scoring logic)
    const prompt = req.body.prompt || (() => {
      ...
      const levelsDesc = (scoringGuide?.levels || [])
        .map(l => `- ${l.score} Punkte (${l.label}): ${l.description}`)
        .join('\n');
      return `Du bist Experte für AI Leadership Assessment. Bewerte die Antwort in ZWEI Schritten.\n\n--- SCHRITT 1: KERNELEMENTE PRÜFEN ---\nGehe jedes Bewertungskriterium einzeln durch und notiere intern ob es KLAR VORHANDEN, NUR ANGEDEUTET oder FEHLEND ist. Sei streng: angedeutet zählt nicht als vorhanden.\n\nBEWERTUNGSSCHEMA:\nMaximale Punkte: ${points}\nBewertungskriterien:\n${criteriaList}\nPunktestufen:\n${levelsDesc}\n\n--- SCHRITT 2: PUNKTZAHL ABLEITEN ---\nLeite die Punktzahl NUR aus Schritt 1 ab. WICHTIG: Gute Formulierung erhöht die Punktzahl NICHT. Nur Inhalt zählt.\nOrthographie- und Grammatikfehler haben keinen Einfluss. Nur wenn eine Antwort syntaktisch so fragmentiert ist, dass kein eindeutiger Sinn erkennbar ist, wähle die inhaltlich ungünstigere Interpretation.\n\nFrage: ${questionText}\nAntwort: ${answerText || '(keine Antwort)'}\n\nAntworte NUR mit JSON ohne Backticks:\n{"score": <0 bis ${points} in 0.5-Schritten>, "reasoning": "<2-3 Sätze>", "strengths": ["<Stärke1>"], "improvements": ["<Verbesserung1>"]}`;
    })();
